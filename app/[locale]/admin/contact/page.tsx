...
export default function ContactList() {
	...
	// Thêm state cho modal confirm xóa
	...
	const handleDelete = async (contactId: number | string) => {
		try {
			await axiosInstance.delete(`/api/contacts?id=${contactId}`);
			setContacts((prev) => prev.filter((contact) => contact.id !== contactId));
			// Đóng cả 2 modal khi xóa thành công
	...
	return (
		<AdminLayout pageName="Contact">
						...
						{contacts.map((contact) => (
							...
							>
								...
								<td className="px-6 py-4 whitespace-nowrap">
									{contact.ngaytao
										? new Date(contact.ngaytao).toLocaleString("en-GB", {
												hour: "2-digit",
												minute: "2-digit",
												day: "2-digit",
												month: "2-digit",
			...
			{isConfirmModalOpen && selectedContact && (
				<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
